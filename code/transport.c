		      ...
		      lead_para *leadsparams, trans_params *tpara, int mode, double *ldoses, double ***currents)
{
	...
	//Calculate the transmissions between all the leads	
		...
		double _Complex tempt=0, tempt2=0, temptud=0, temptdu=0;
...
//z-spin				
// 				MatrixCopyPartM(Gamma, Gamma1, 1.0, s1, s1, 0, 0, d1, d1);
// 				MatrixCopyPartM(Gamma, Gamma1, -1.0, tcell1dim+s1, tcell1dim+s1, d1, d1, d1, d1);	
// 				MatrixCopyPartM(Gamma, Gamma1, 1.0, s1, tcell1dim+s1, 0, d1, d1, d1);	
// 				MatrixCopyPartM(Gamma, Gamma1, -1.0, tcell1dim+s1, s1, d1, 0, d1, d1);
				
//x-spin				
// 				MatrixCopyPartM(Gamma, Gamma1, 1.0, s1, s1, d1, 0, d1, d1);
// 				MatrixCopyPartM(Gamma, Gamma1, 1.0, s1, tcell1dim+s1, d1, d1, d1, d1);	
// 				MatrixCopyPartM(Gamma, Gamma1, 1.0, tcell1dim+s1, s1, 0, 0, d1, d1);		
// 				MatrixCopyPartM(Gamma, Gamma1, 1.0, tcell1dim+s1, tcell1dim+s1, 0, d1, d1, d1);
				
//y-spin				
// 				MatrixCopyPartM(Gamma, Gamma1, I, s1, s1, d1, 0, d1, d1);
// 				MatrixCopyPartM(Gamma, Gamma1, I, s1, tcell1dim+s1, d1, d1, d1, d1);	
// 				MatrixCopyPartM(Gamma, Gamma1, -I, tcell1dim+s1, s1, 0, 0, d1, d1);		
// 				MatrixCopyPartM(Gamma, Gamma1, -I, tcell1dim+s1, tcell1dim+s1, 0, d1, d1, d1);
				...
				//printEMatrix(Gamma1, 2*d1);
					...
					for(j=0; j< num_leads; j++)
					{
						...
						for(k=0; k<d1; k++)
						{
							for(l=0; l<d2; l++)
							{
								tempt +=temp2[k][l] * gsa[l][k];
								tempt2 += temp2[d1+k][d2+l] * gsa[d2+l][d1+k];
								temptud +=  temp2[k][d2+l] * gsa[d2+l][k];
								temptdu +=  temp2[d1+k][l] *gsa[l][d1+k];
							}
						}
						...
						(tpara->transmissions)[i][j] = creal(tempt +tempt2 + temptud + temptdu);
						//printf("# %lf	%lf	%lf	%lf\n", tempt ,tempt2 , temptud , temptdu);
						FreeMatrix(temp2);
						
						
						s2+=d2;
						 FreeMatrix(Gamma2); FreeMatrix(gsa);
...
//   						printf("%d cell \n#g00inv\n", it_count);
//   						listNonZero(g00inv, 2*dim, 2*dim);
//   						if(it_count>0)
//   						{
//  							  printf("\n#V21\n");
...
void multipleLeads (double _Complex En, RectRedux *DeviceCell, RectRedux **LeadCells, cellDivision *cellinfo, lead_para *params, double _Complex **Sigma)
{
    ...
    int leadloop, dim1, dim1a, dim2, dim2a,  dimcounta=0, lcount;
    double _Complex **ginv, **V12, **V21, **g00, **SL, **SR, **VLD, **VDL, **smallSigma, **temp1, **temp2;
    ...
    lead_para *ribpara = (lead_para *)params;
    gen_hop_params *hoppingparams = (gen_hop_params *)(ribpara->hoppara);

    int spindep = DeviceCell->spindep;
    int leadspindep, sumspin, leadspinpol, spinpolsign;
	...
	if(spindep == 0 || spindep ==1 || leadspindep == 0)
	{
			...
			for(s2=0; s2<sumspin; s2++)
			{
				...
				for(i=0; i <dim1; i++)
				{
					for(j=0; j<dim1a; j++)
					{
						...
						VLD[s1*dim1+i][s2*dim1a+j] =  (hopfn)(LeadCells[leadloop], DeviceCell, i, k, bshifts0, (params->hoppara) );
						
						VDL[s1*dim1a+j][s2*dim1+i] =  (hopfn)(DeviceCell, LeadCells[leadloop], k, i, bshifts0, (params->hoppara) );
					}
				}
			}
		}
		...
		FreeMatrix(temp1); FreeMatrix(VLD); FreeMatrix(VDL); FreeMatrix(SL);
	...
	{	
		...
		for(s1=0; s1<2; s1++)
		{
			...
			//generate the info required for Rubio method
			lead_prep_mag(En, LeadCells[leadloop], leadloop, params, ginv, V12, V21, s1);
			...
			for(i=0; i <dim1; i++)
			{
				...
				{
					k=(cellinfo->lead_sites)[dimcounta + j];
					VLD[i][j] =  (hopfn)(LeadCells[leadloop], DeviceCell, i, k, bshifts0, hoppingparams );
					VDL[j][i] =  (hopfn)(DeviceCell, LeadCells[leadloop], k, i, bshifts0, hoppingparams );
				}
				
			}
			
			temp1 = createNonSquareMatrix(dim1a, dim1);
			temp2=createSquareMatrix(dim1a);
			MatrixMultNS(VDL, SL, temp1, dim1a, dim1, dim1);
			MatrixMultNS(temp1, VLD, temp2, dim1a, dim1, dim1a);
			
			
			MatrixCopyPart(temp2, smallSigma, 0, 0, s1*dim1a, 0, dim1a, dim1a);
			
			FreeMatrix(temp1); FreeMatrix(VLD); FreeMatrix(VDL); FreeMatrix(temp2); FreeMatrix(SL);
